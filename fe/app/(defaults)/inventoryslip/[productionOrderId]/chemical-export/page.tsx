...
const ChemicalExportSlipPage = () => {
    ...
    const loadProductionOrderInfo = async () => {
        ...
        try {
            ...
            if (info) {
                // Kiểm tra loại lệnh sản xuất có phải là "Sản xuất keo" hoặc "Đổ keo" không
                if (!['Sản xuất keo', 'Đổ keo'].includes(info.type)) {
                    alert('Lệnh sản xuất này không phải là loại "Sản xuất keo" hoặc "Đổ keo"');
    ...
    const handleSlipUpdated = async (slip: any) => {
        try {
            ...
            alert('Cập nhật phiếu xuất hóa chất thành công!');
            router.push(`/inventoryslip/${productionOrderId}`);
        } catch (error) {
    ...
    return (
        <div>
            <div className="mb-6">
                <h1 className="text-2xl font-bold mb-2">Tạo phiếu xuất hóa chất</h1>
...
export default ChemicalExportSlipPage;
