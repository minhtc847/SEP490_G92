...
interface ProductionPlanListItem {
    id: number;
    planDate: string;
    title: string;
    description: string;
...
const statusBadgeMap: Record<string, string> = {
    '0': 'badge-outline-warning', // Draft
    '1': 'badge-outline-info',    // Active
...
const ProductionPlanListAccountant: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const mockPlans: ProductionPlanListItem[] = [
            {
                id: 1,
                planDate: new Date().toISOString(),
                title: 'Kế hoạch sản xuất tháng 12/2024',
                description: 'Kế hoạch sản xuất kính cường lực cho các đơn hàng',
                status: '1',
            ...
            },
            {
                ...
                title: 'Kế hoạch sản xuất tháng 11/2024',
                description: 'Kế hoạch sản xuất kính cường lực cho các đơn hàng',
                status: '2',
                totalValue: 98000000
            }
        ...
        const sorted = [...mockPlans].sort((a, b) => new Date(b.planDate).getTime() - new Date(a.planDate).getTime());
        setPlans(sorted);
        setLoading(false);
    }, []);
    ...
    useEffect(() => {
        ...
        setRecords([...plans.slice(from, to)]);
    }, [plans, page, pageSize]);

    return (
        <div>
            <div className="mb-5">
                <h1 className="text-2xl font-bold">Danh sách kế hoạch sản xuất (Accountant View)</h1>
                <p className="text-gray-600 mt-2">Quản lý và theo dõi kế hoạch sản xuất từ góc độ kế toán</p>
            </div>
                    ...
                    <table className="table-striped">
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={7} className="text-center py-4">
                                        Không có dữ liệu kế hoạch sản xuất
                                    </td>
                                </tr>
                            ) : (
                                records.map((item, idx) => (
                                    <tr key={item.id}>
                                        ...
                                        <td>{item.planDate ? new Date(item.planDate).toLocaleDateString() : '-'}</td>
                                        <td>{item.title}</td>
                                        <td>{item.description}</td>
                                        <td style={{ minWidth: '150px' }}>
                                            <span className={`badge ${statusBadgeMap[item.status || ''] || 'badge-outline-secondary'} whitespace-nowrap`}>
                                                {getStatusText(item.status)}
                                            </span>
                                        </td>
                                        <td>{item.totalValue.toLocaleString('vi-VN')} VNĐ</td>
                                        <td>
                                            <Link
                                                href={`/mockup/accountant/production-plans/${item.id}`}
