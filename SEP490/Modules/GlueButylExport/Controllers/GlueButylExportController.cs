...
namespace SEP490.Modules.GlueButylExport.Controllers
{
    ...
    public class GlueButylExportController : ControllerBase
    {
        ...
        [HttpGet("get-all/{productionOrderId}")]
        public async Task<IActionResult> GetAllExportsByProductionOrderId(int productionOrderId)
        {
            try
            {
                var exports = await _glueButylExportService.getAllExportByProductionOrderId(productionOrderId);
                return Ok(exports);
            }
            catch (Exception ex)
            {
